        ...
        appendTransaction: (transactions, oldState, newState) => {
          ...
          doc.descendants((node, pos) => {
            ...
            if (node.type.name === "text" && node.marks.some(mark => mark.type.name === "link")) {
              ...
              const wasLink = oldNode?.marks.some(mark => mark.type.name === "link" && mark.attrs.href === url);
              ...
              if (!wasLink && url) {
                ...
                const hasPreview = nextNode?.type.name === "linkPreview" && 
                                 nextNode?.attrs.url === url;
                
                if (!hasPreview) {
                  console.log('[AutoLinkPreview] Adding preview for URL:', url);
